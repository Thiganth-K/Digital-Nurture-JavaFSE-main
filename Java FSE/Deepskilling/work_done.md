...
| **Day 1** | Spring Data JPA | Quick Example: Spring Boot, Logger Levels & H2 Context Load | **Completed** | `orm-learn` | Spring Boot 3, Maven, H2 Database, SLF4J |
| **Day 1** | Spring Data JPA | Entity Mapping: Country Table & Retrieve All | **Completed** | `orm-learn` | `@Entity`, `@Table`, `@Column`, `JpaRepository` |
| **Day 1** | Spring Data JPA | Find Country by Code (with custom exception handlings) | **Completed** | `orm-learn` | `findById()`, `Optional<T>`, Custom Exceptions |
| **Day 1** | Spring Data JPA | Add New Country & Entity Persistence | **Completed** | `orm-learn` | `save()`, SQL INSERT bindings, H2 Persistence |
| **Day 1** | Spring Data JPA | Update Country details | **Completed** | `orm-learn` | SQL UPDATE binding, Dirty Checking, Transactional |
| **Day 1** | Spring Data JPA | Delete Country | **Completed** | `orm-learn` | `deleteById()`, SQL DELETE execution |
| **Day 1** | Spring Data JPA | Custom Query Methods (Partial Match search & alphabetical ordering) | **Completed** | `orm-learn` | JPA Query Method DSL (`ContainingOrderByNameAsc`) |
| **Day 1** | Spring Data JPA | Object-Relational Mapping (O/R Mapping) - 1:N, N:1, N:M | **Completed** | `orm-learn` | `@OneToMany`, `@ManyToOne`, `@ManyToMany`, `@JoinTable` |
| **Day 1** | Spring Data JPA | Advanced JPQL/HQL and Native Queries | **Completed** | `orm-learn` | `@Query`, `@Param`, Native SQL Bindings, HQL |
| **Day 2** | Spring REST | Creating Spring REST Web Project using Maven & Hello World Endpoint | **Completed** | `country-rest-service` | Spring Web, `@RestController`, `@GetMapping` |
| **Day 2** | Spring REST | REST Country Service (Retrieve all countries) | **Completed** | `country-rest-service` | REST endpoints, JSON serialization |
| **Day 2** | Spring REST | REST Get country based on code | **Completed** | `country-rest-service` | Path Variables (`@PathVariable`), HTTP status codes |
| **Day 2** | Spring REST | REST CRUD operations: Add, Update, and Delete Country | **Completed** | `country-rest-service` | `@PostMapping`, `@PutMapping`, `@DeleteMapping` |
| **Day 2** | Spring REST | JWT-Secured REST API & Token Authentication | **Completed** | `country-rest-service` | Spring Security, JWT Token Filter, Authorization Header |
| **Day 2** | Microservices | Service Discovery: Create Eureka Discovery Server & register services | **Completed** | `eureka-server` | Spring Cloud Netflix Eureka Server, `@EnableEurekaServer` |
| **Day 2** | Microservices | API Gateway Routing: Dynamic routing & Load Balancing | **Completed** | `api-gateway` | Spring Cloud Gateway, Load Balancer (`lb://`), `eureka-client` |
